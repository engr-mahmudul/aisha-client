...
import BurgerCard, { FoodCardProps } from "@/components/BurgerCard";
type MenuItem = {
  id: number;
  englishName: string;
  finnishName: string;
          ...
          >
            ...
            <div className="relative z-10 max-w-2xl w-[92vw] sm:w-[85vw] md:w-[720px] rounded-2xl bg-white shadow-xl border border-gray-200 overflow-hidden">
              ...
              "
                  ...
                  {typeof selected.burgerPrice === "number"
                    ? new Intl.NumberFormat(undefined, {
                        style: "currency",
                        currency: selected.currency || "EUR",
                        maximumFractionDigits: 2,
                      }).format(selected.burgerPrice)
                    : selected.burgerPrice}
                </div>
                {selected.ateriaPrice && (
                  <div className="text-xl font-bold text-red-500">
                    <span
                      className="text-black text-sm
              "
                    ...
                    {typeof selected.ateriaPrice === "number"
                      ? new Intl.NumberFormat(undefined, {
                          style: "currency",
                          currency: selected.currency || "EUR",
                          maximumFractionDigits: 2,
                        }).format(selected.ateriaPrice)
                      : selected.ateriaPrice}
                  </div>
                )}
                <button
